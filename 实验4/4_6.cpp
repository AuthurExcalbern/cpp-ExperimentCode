...
{
    list<string> ret;
    stack<Rule> stack_rule;
    //gen_aux(g, "<sentence>", ret);
    
    string word = "<sentence>";
    Grammar::const_iterator first_iter = g.find(word);
    const Rule_collection first_c = first_iter->second;
    const Rule first_r = *(first_c.begin());
    
    stack_rule.push(first_r);
    
    while(!stack_rule.empty())
    {
        const Rule now_rule = stack_rule.top();
        stack_rule.pop();
        
        for(Rule::const_iterator i = now_rule.begin(); i != now_rule.end(); i++)
        {
            if(!bracketed(*i))
            {
                ret.push_back(*i);
            ...
            {
                Grammar::const_iterator iter = g.find(*i);
                if(iter == g.end())
